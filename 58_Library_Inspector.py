...
def convert_md_to_html(md_content: str, title: str) -> str:
    ...
    # 【关键修复】更新 Mermaid 配置
    # useMaxWidth: false -> 防止图表被压缩，允许超出容器宽度
    html_template = f"""
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .edgeLabel {{
            ...
            background-color: rgba(255, 255, 255, 0.9) !important; /* 半透明白色背景遮挡线条 */
    ...
    <script type="module">
        ...
        mermaid.initialize({{ 
            ...
            flowchart: {{ 
                ...
                rankSpacing: 150, 
                nodeSpacing: 100,
                // 曲线风格，有时候 basis 比 linear 更不容易重叠
                curve: 'basis' 
            }} 
        }});
    </script>
...
def inspect_library(
    ...
    for u, v in source_data:
        if u in top_nodes or v in top_nodes:
            # 提取短名称用于 Label
            short_u_label = u.replace(library_name + ".", "").split('.')[-1]
            short_v_label = v.replace(library_name + ".", "").split('.')[-1]
            if not v.startswith(library_name): 
                short_v_label = v.split('.')[0]
            
            if short_u_label == short_v_label: continue
            
            # 【关键修复】添加 _node 后缀，防止 ID 与 Mermaid 关键字（如 Module, Graph）冲突
            u_id = sanitize_id(u) + "_node"
            v_id = sanitize_id(v) + "_node"
            ...
            # 使用安全 ID 构建边
            mermaid_lines.append(f'    {u_id}["{short_u_label}"]{arrow}{v_id}["{short_v_label}"]')
            
            if u.startswith(library_name): mermaid_lines.append(f"    class {u_id} core;")
            else: mermaid_lines.append(f"    class {u_id} external;")
            
            if v.startswith(library_name): mermaid_lines.append(f"    class {v_id} core;")
            else: mermaid_lines.append(f"    class {v_id} external;")
    ...
    if len(all_classes) < 50: 
        for name, obj in all_classes:
            for base in obj.__bases__:
                ...
                # 【关键修复】添加 _node 后缀
                cls_id = sanitize_id(name) + "_node"
                base_id = sanitize_id(base_name) + "_node"
                
                mermaid_lines.append(f'    {cls_id}["{name}"] --|> {base_id}["{base_name}"]')
                mermaid_lines.append(f"    class {cls_id} core;")
                
                if base.__module__.split('.')[0] != library_name: 
                    mermaid_lines.append(f"    class {base_id} external;")
                else: 
                    mermaid_lines.append(f"    class {base_id} core;")
