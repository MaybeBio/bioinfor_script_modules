...
# 4, cell+未修改
    ...
    def get_charge_blocks_1(self,
        ...
        for i in range(len(sequence) - window_size + 1):
            ...
            if abs(net_charge) >= NCPR_threshold:
                if net_charge > 0:
                    # 保留窗口起点、终点、窗口大小、窗口内序列、窗口内序列净电荷值、NCPR值、窗口类型
                    # 此处起点、终点为0-based index，后续合并时再统一转换为1-based index
                    Basic_blocks.append((i,i+window_size-1,window_size,window_seq,net_charge,NCPR,"Basic"))

                else:
                    Acidic_blocks.append((i,i+window_size-1,window_size,window_seq,net_charge,NCPR,"Acidic"))
        ...
        if Acidic_blocks:
            # 非空则取第1个窗口，作为后续比对起点
            Acidic_block_merged.append(Acidic_blocks[0])
            for current_acidic in Acidic_blocks[1:]:
                last_acidic = Acidic_block_merged[-1]
                ...
                if current_acidic[0] <= last_acidic[1] + 1:
                    ...
                    merged_end = max(last_acidic[1],current_acidic[1])
                    merged_seq = sequence[merged_start:merged_end + 1]
                    merged_net_charge = sum([self.aa_charge.get(aa,0) for aa in merged_seq])
                    merged_size = len(merged_seq)
                    if merged_size > 0:
                        merged_NCPR = merged_net_charge / merged_size
                    else:
        ...
        if Basic_blocks:
            # 非空则取第1个窗口，作为后续比对起点
            Basic_block_merged.append(Basic_blocks[0])
            for current_basic in Basic_blocks[1:]:
                last_basic = Basic_block_merged[-1]
        ...
        return {
            "Acidic_blocks":Acidic_block_merged,
            "Basic_blocks":Basic_block_merged,
            "Charge_blocks": sorted(Acidic_block_merged + Basic_block_merged, key=lambda x: x[0])  # 按起点升序排序
        }  
